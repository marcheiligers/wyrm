function syncDataFiles(dbname, baseurl) {
  ...
  var succeeded = function () {
    ...
    state.reported_result = true;
    var why =
      "File data synchronized (downloaded " +
      Math.ceil(state.total_downloaded / 1048576) +
      " megabytes in " +
      state.total_files +
      " files)";
    log("[SUCCESS] " + why);
  ...
  dbopen.onupgradeneeded = function (event) {
    ...
    var metadataStore = db.createObjectStore("metadata", {
      keyPath: "filename",
    });
    var dataStore = db.createObjectStore("data", {
      keyPath: "chunkid",
      autoIncrement: true,
    });
  ...
  dbopen.onerror = function (event) {
    ...
    var errorMessage =
      "Couldn't open local database: " + event.target.error.message;
    if (isBraveBrowser) {
      errorMessage +=
        "<br/>Note: This error can also happen if you are using Brave Browser and have not disabled Shield (click the Lion icon in the address bar and disable Shield for this site).";
    }
    failed(errorMessage);
  };
  ...
  var download_another_file = function () {
    ...
    xhr.addEventListener("progress", function (e) {
      ...
      var percent = state.total_to_download
        ? Math.floor((state.total_downloaded / state.total_to_download) * 100.0)
        : 0;
      progress(
        "Downloaded " +
          percent +
          ...
          Math.ceil(state.total_downloaded / 1048576) +
          "/" +
          Math.ceil(state.total_to_download / 1048576) +
          " megabytes)",
      );
    });
    ...
    xhr.addEventListener("load", function (e) {
      ...
      if (xhr.status != 200) {
        ...
        var percent = state.total_to_download
          ? Math.floor(
              (state.total_downloaded / state.total_to_download) * 100.0,
            )
          : 0;
        progress(
          "Downloaded " +
  ...
  var delete_old_files = function () {
    ...
    for (var i in state.local_manifest) {
      ...
      if (typeof state.remote_manifest[localfname] === "undefined") {
        ...
        if (
          localitem.filesize != remoteitem.filesize ||
...
var prepareFilesystem = function () {
  ...
  FS.syncfs(true, function (err) {
    if (err) {
      console.log(
        "WARNING: Failed to populate persistent store. Save games likely lost?",
      );
    } else {
    ...
    for (var i in manifest) {
      ...
      req.onsuccess = function (event) {
        ...
        try {
          ...
          // throws if file exists, etc. Nuke and try one more time.
          FS.unlink(path);
          try {
            okay = FS.createDataFile(
              basedir,
        ...
        if (!okay) {
          ...
          var percent = Math.floor(
            (completed / syncdata.total_requests) * 100.0,
          );
...
var Module = {
  ...
  startClickToPlay: function () {
    ...
    div.style.backgroundColor = "#eeeee6";
...
Module.onRuntimeInitialized = function () {
  Module.ffi_send = Module.cwrap("ffi_js_send", "string", ["string", "string"]);

  Module.ffi_free = function (s) {
    Module.ccall("ffi_js_free_string", null, ["string"], [s]);
  };
};
...
function loadLoadMainModule() {
  ...
  if (!hasWebAssembly) {
    Module.setStatus(
      "Your browser doesn't have WebAssembly support. Please upgrade.",
    );
  } else if (!hasSharedArrayBuffer) {
    var isBraveBrowser = navigator.brave || false;
    var isHttps = window.location.protocol === "https:";
    var errorMessage =
      "Your browser doesn't have SharedArrayBuffer support. Please upgrade, or make sure the webserver set proper COOP/COEP headers!";
    if (!isHttps) {
      errorMessage +=
        "<br/>Note: This error can also happen if you are not using HTTPS.";
    }
    if (isBraveBrowser) {
      errorMessage +=
        "<br/>Note: This error can also happen if you are using Brave Browser and have not disabled Shield (click the Lion icon in the address bar and disable Shield for this site).";
    ...
    if ("serviceWorker" in navigator) {
      ...
      navigator.serviceWorker.register("dragonruby-serviceworker.js").then(
        function (registration) {
          console.log(
            "DragonRuby COOP/COEP Service Worker registered",
            registration.scope,
          );
          // If the registration is active, but it's not controlling the page
          if (!navigator.serviceWorker.controller) {
            Module.setStatus(
              "One moment, reloading to enable SharedArrayBuffer.",
            );
...
function resizeCanvasWindowsChrome() {
  ...
  if (
    Math.floor(currentWindowWidth / 16) < Math.floor(currentWindowHeight / 9)
  ) {
